...
public sealed class AnnualSimulationComponent : GH_Component
{
    ...
    protected override void SolveInstance(IGH_DataAccess da)
    {
        ...
        if (!da.GetData(0, ref root) || !da.GetData(1, ref epw)) return; da.GetData(2, ref sky); da.GetData(3, ref detail); da.GetData(4, ref run);
        try
        {
            root = Path.GetFullPath(root); if (!File.Exists(epw)) throw new FileNotFoundException("EPW weather file was not found.");
            var grid = Path.Combine(root, "model", "grid", "0.pts"); var scene = Path.Combine(root, "model", "scene");

...
public sealed class AnnualSimulationComponent : GH_Component
{
    ...
    private static IEnumerable<string> Commands(string epw, string pts, int sensors, int sky, string detail, int part, string? radianceBin) => new[] { "@echo off", string.IsNullOrWhiteSpace(radianceBin) ? "" : $"set \"PATH={radianceBin};%PATH%\"", $"epw2wea \"{epw}\" Weather_{part}.wea", $"gendaymtx -m {sky} Weather_{part}.wea > Weather_{part}.smx", "oconv envelope.mat envelope.rad > amodel.oct", $"rfluxmtx -I+ -y {sensors} {detail} - skyglow.rad -i amodel.oct < {pts} > illum_{part}.mtx", $"dctimestep illum_{part}.mtx Weather_{part}.smx | rmtxop -fa -t -c 47.4 119.9 11.6 - > annualRfinal_part{part}.ill" };
}

...
public class SelectSpectralFactorComponent : FlahaGrowComponent
{
    ...
    protected override void SolveInstance(IGH_DataAccess da) { var run = false; var step = 1; da.GetData(0, ref run); da.GetData(1, ref step); if (run) ShowPicker(Math.Max(1, step)); WarnIfSourceChanged(); da.SetData(0, _factor); da.SetData(1, _label); }

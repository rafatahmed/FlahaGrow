...
{
    public HourlyParComponent() : base("Hourly PAR", "Hourly PAR", "Reads one annual-cache hour and converts every sensor value from lux to PPFD.", "FlahaGrow", "05 PPFD") { }
    ...
    protected override void SolveInstance(IGH_DataAccess da)
    {
        ...
        try { var factor = AnnualCacheData.Factor(factorInput); var ppfd = PlantLightMath.ConvertLux(AnnualCacheData.Hour(cache, hour), factor).ToList(); da.SetDataList(0, ppfd); da.SetData(1, $"OK: hour {hour} → {ppfd.Count} PPFD values; factor {factor:0.####}."); }
...
{
    public ParEachSensorComponent() : base("PAR Each Sensor", "Sensor PAR", "Reads one sensor's annual cache column, converts it to PPFD, and can return its marker.", "FlahaGrow", "05 PPFD") { }
    ...
    protected override void SolveInstance(IGH_DataAccess da)
    {
        ...
        try
        {
            var factor = AnnualCacheData.Factor(factorInput); var ppfd = PlantLightMath.ConvertLux(AnnualCacheData.Sensor(cache, sensor), factor).ToList(); da.SetDataList(0, ppfd);

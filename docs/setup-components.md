...
The annual runner expects a Honeybee ModelToRad root containing `model/scene` and a sensor grid or supplied points. Connect Working Directory's `Analysis` to its optional Analysis input to own the run. Preparation copies inputs into a new isolated run folder; connect the runner's `Folder` output to progress and cache. Setup's Inputs and Runs containers are not Honeybee source roots. See [annual run isolation](annual-run-isolation.md) for wiring, legacy-result migration, and remaining validation limits.

See the [post-Setup component consistency review](setup-consistency-review-2026-09-07.md) for the connection matrix and remaining lifecycle/migration validation; use [current status](current-status.md) for the installed build and open findings.

Annual validation update (2026-09-08): prepare a new run for the schema-2 manifest and checked command states. Progress now reports Completed only after commands succeed and the final matrix passes header, dimension, finite-value and nonnegative-illuminance validation (updated September 10). Failed commands stop their part and record diagnostics; caches reject failed or invalid results. Earlier schema-1 runs require regeneration. User-reported live Setup readiness is confirmed; full annual study validation remains a separate check.

...
Original increment: 2026-09-07. This document preserves its implementation evidence; it is not the active Setup or annual-run specification. Use the [documentation index](../../README.md), [Setup guide](../getting-started/setup.md), and [current status](../quality/current-status.md) for current behavior.

These notes describe the first shared-core increment. Subsequent increments implemented [workspaces](setup-workspaces.md), [Radiance checks](setup-radiance.md), and [Grasshopper Setup components](../getting-started/setup.md). The annual-run contract is now documented in [annual run isolation](../workflows/annual-run-isolation.md); its outstanding validation is tracked in [current status](../quality/current-status.md).
